...
def login():
    try:
        response = requests.post(f"http://localhost:5000/register_peer", json={"peer_address": "localhost:5002", "peer_files": local_files})
        if response.status_code == 200:
            data = response.json()
            neighbor_peers = data.get('registered_peers')
            for peer in neighbor_peers.keys():
                if(peer != "localhost:5002"):
...
def update_neighbors():
    ...
    for new_neighbor in new_neighbors.keys():
        if new_neighbor not in neighbors_peers_files.keys() and new_neighbor != 'localhost:5002':
            neighbors_peers_files[new_neighbor] = new_neighbors[new_neighbor]
    return jsonify({"message": "Neighbors updated successfully"}), 200
    
@app.route('/')
def home():
    return "You can register yourself as a peer at http://localhost:5002/login"
